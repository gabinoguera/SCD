...
#### Agent Usage Pattern
Agents are invoked by the Project Manager based on detected tech stack. Each agent:
- Researches and proposes implementation plans
- Creates documentation in `openspec/changes/{change-id}/doc/{agent}.md`
...
- User requests feature: "Add user authentication"
- Project Manager creates `openspec/changes/{change-id}/`
- PM generates `proposal.md`, `tasks.md`, `specs/`

#### Phase 1: Planning
- Project Manager reads `agent-resources.yaml`
- Detects tech stack requirements for feature
- Creates `openspec/changes/{change-id}/doc/` directory
- Orchestrates consultant agents in parallel
- Agents write plans in `openspec/changes/{change-id}/doc/`

#### Phase 2: Implementation
- Read all plans in `openspec/changes/{change-id}/doc/`
- Implement following the detailed specifications
- Update `tasks.md` checklist as you progress

#### Phase 3: Validation
- Use `qa-criteria-validator` for final validation
- Review report in `openspec/changes/{change-id}/doc/validation-report.md`
...
- `.claude/research/`: Phase 0 ideation outputs (from `/ideation`)
...
- `openspec/changes/{change-id}/doc/`: Agent-generated implementation plans

### Configuration
- `agent-resources.yaml`: Single source of truth (tech stack → agents mapping)
- `.mcp.json`: Model Context Protocol configuration (auto-generated by PM)
- `project-state.json`: Detected tech stack (auto-generated by PM)
...
2. **Agent Orchestration**: Project Manager coordinates all consultant agents
3. **Documentation Location**: All agent plans go in `openspec/changes/{change-id}/doc/`
4. **No Direct Implementation**: Agents propose plans, parent agent implements
5. **Validation Required**: Always run `openspec validate --strict` before submission

## Subagent Management

You have access to 14+ specialized subagents. When working on features:

1. **Let Project Manager orchestrate** - Say "Add [feature]" to trigger PM
2. **PM reads agent-resources.yaml** to determine which agents to call
3. **Agents write to** `openspec/changes/{change-id}/doc/`
4. **Read all plans** before implementing
...
- **agent-resources.yaml**: Single source of truth for tech stack → agents mapping

**Auto-Generated Files:**
- `.mcp.json`: Generated by Project Manager based on `agent-resources.yaml`
- `project-state.json`: Generated by Project Manager during initialization
